...
<body style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <div class="container text-center">
        <div class="row row-cols-3" style='margin-top: 1rem;'>
            ...
            $dresses_result = mysqli_query($DBConnect, "SELECT * FROM dresses, designers WHERE dresses.category_id='$category_id' AND dresses.designer_id=designers.designer_id");
            if (mysqli_num_rows($dresses_result) > 0) {
                while ($dresses_row = mysqli_fetch_assoc($dresses_result)) {
                    $image = $dresses_row['image'];
                    $name = $dresses_row['name'];
                    $description = $dresses_row['description'];
                    $rental_price = $dresses_row['rental_price'];
                    $size = $dresses_row['size'];
                    echo "
                <div class='col'>
                    <div class='image-container'>
                        <a href='dress_details.php?dress_code=" . $dresses_row['dress_code'] . "'><img src='images/" . $image . "' style='width: 80%; height: 420px; object-fit: cover; border-radius: 5%;'></a>
                    </div>
                    <p class='dress_designer_p'>" . $name . "</p>
                    <p class='dress_description_p'>" . $description . "</p>
                    <p class='dress_price_p'>Rental price: $" . $rental_price . "</p>
                    <p class='dress_size_p'>Size: " . $size . "</p>
                </div>
                ";
                }
            } else {
                $cat_sql = mysqli_query($DBConnect, "SELECT * FROM categories WHERE category_id='$category_id'");
                if ($cat_row = mysqli_fetch_assoc($cat_sql)) {
                    $name = $cat_row['name'];
                    echo "<div class='col-12' style='margin-top: 2rem;'>";
                    echo "<h4 style='margin-top: 7rem;'>No dresses available under " . $name . " occasion.</h4>";
                    echo "</div>";
                }
            }
