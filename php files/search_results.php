...
<body style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <div class="container text-center">
        <div class="row row-cols-3" style='margin-top: 1rem;'>
            ...
            $searchQuery = $DBConnect->real_escape_string($searchQuery);

            // Queries for each table
            $queries = [
                "SELECT *, categories.name AS category_name, designers.name AS designer_name 
                FROM dresses 
                LEFT JOIN categories ON dresses.category_id = categories.category_id 
                LEFT JOIN designers ON dresses.designer_id = designers.designer_id 
                WHERE dresses.color LIKE '%$searchQuery%'",
                "SELECT *, categories.name AS category_name, designers.name AS designer_name 
                FROM dresses 
                LEFT JOIN categories ON dresses.category_id = categories.category_id 
                LEFT JOIN designers ON dresses.designer_id = designers.designer_id 
                ...
                LEFT JOIN categories ON dresses.category_id = categories.category_id 
                LEFT JOIN designers ON dresses.designer_id = designers.designer_id 
                WHERE designers.name LIKE '%$searchQuery%'"
            ];
            ...
            if (count($results) > 0) {
                foreach ($results as $row) {
                    ...
                    echo "<p class='dress_designer_p'><a href='designer_index.php?designer_id=" . $row['designer_id'] . "' title='" . $row['designer_name'] . "'>" . $row['designer_name'] . "</a></p>";
